...
$api->get('/get/nodes/community/:communityID/new/format/geojson', function($communityID) use ($api) {
	...
	$result = dbSQL('SELECT id FROM '. TBL_NODES .' WHERE community = "'. $community->getID() .'" AND firstseen > "'. $timestamp .'" AND firstseen != "0" ORDER BY id ASC');
